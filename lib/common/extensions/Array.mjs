...
Array.prototype.noDup = function(selector) {
    return this.reduce((unique, item) =>
        unique
            .map((element) => selector(element))
            .includes(selector(item)) ? unique : [...unique, item],
        []);
};

// eslint-disable-next-line no-extend-native
Array.prototype.sortDescending = function(selector) {
    return this.sort((left, right) => selector(right) - selector(left));
};
