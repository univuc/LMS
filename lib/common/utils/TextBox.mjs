...
class TextBox {
    ...
    _setWidthToFitContents() {
        const arrayOfLengths = [...this.content, this.title].map((str) => str.length);
        const maxLength = Math.max(...arrayOfLengths) + 2;
    ...
    _printStartLine() {
        const startLineLength = this.width - this.title.length - 2;
    ...
    _printMidLines() {
        for (const line of this.content) {
            const afterPadding = this.width - line.length - 3;
