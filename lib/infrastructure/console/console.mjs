...
export default async function directExecute() {
...
function showScheduledTasks() {
    const schedules = resolve(Scheduler).viewTasks();
    ...
    for (const entry of schedules.entries()) {
        const tag = entry[0];
        const {userId, clip} = tag;

        content.push(`${userId}: ${clip.title} after ${clip.runningTime} sec`);
    }
    ...
    textBox.setTitle(`Scheduled tasks`);
