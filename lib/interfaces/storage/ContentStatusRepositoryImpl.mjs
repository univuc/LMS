...
class ContentStatusRepositoryImpl extends ContentStatusRepository {
    ...
    async isAssignmentCleared(userId, assignment) {
        ...
        switch (status) {
            case config.assignments.cleared_label:
                return true;

            case config.assignments.not_cleared_label:
                return false;
    ...
    async isClipCleared(userId, clip) {
        return await this.getClipProgress(userId, clip) > config.clips.threshold;
    }
    ...
    async getClipProgress(userId, clip) {
        ...
        switch (clip.type) {
            case 'vod':
                return await this._getVodProgress(agent, clip);

            case 'econtents':
                return await this._getEcontentsProgress(agent, clip);

            case 'xncommons':
                return await this._getXncommonsProgress(agent, clip);
        }
    }
