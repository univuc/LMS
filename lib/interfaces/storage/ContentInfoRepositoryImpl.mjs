...
// eslint-disable-next-line no-unused-vars
import * as Array from '../../common/extensions/Array';
...
class ContentInfoRepositoryImpl extends ContentInfoRepository {
    ...
    async getCourses(userId) {
        ...
        return courses.noDup((course) => course.id);
    }
    ...
    async getAssignments(userId, courseId) {
        ...
        const assignments = instances
            .map((instance) => new Assignment(instance));

        return assignments
            .noDup((assignment) => assignment.id)
            .sortDescending((assignment) => assignment.id);
    }
    ...
    async getClips(userId, courseId) {
        ...
        const clips = instances
            .map((instance) => new Clip(instance));

        return clips
            .noDup((clip) => clip.id)
            .sortDescending((clip) => clip.id);
    }
    ...
    _convertMinuteAndSecondStringToSeconds(mmssString) {
        if (mmssString) {
            ...
            return runningTimeMinutesAndSeconds[0] * 60 +
                runningTimeMinutesAndSeconds[1];
        }
    }
}
